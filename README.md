# 極限操控方塊 V2.4.5｜進度一致性・安全確認・結算黑屏修正版

## V2.4.5 修正重點
- 高等級玩家沒有正式存檔時，改由同一筆完整成績快照恢復分數、關卡與累積消行，不再固定退回 Level 21 或把消行歸零。
- 本機龍虎榜、玩家資料與世界盃候選改用完整快照比較，禁止把不同來源的最高分、最高關卡與消行數拼成一筆紀錄。
- 過關後同步保存下一關起點為正式進度，降低停在過關畫面或返回首頁時遺失進度的風險。
- Game Over 先顯示可操作的結算罩幕，再執行排行榜與雲端同步；即使其中一項失敗也不會只剩黑屏。
- 暫停返回首頁、Game Over 全新挑戰、扣分重挑、扣技能重挑與鍵盤 R 重開均加入得失說明及二次確認。
- 全站、統計腳本、PWA 描述及 Open Graph／Twitter 分享資訊統一更新為 V2.4.5。
...
## V2.4.1 公開數據精簡・陪跑漸進・30 關統計

### 玩家前台公開數據
公開看板只保留：
- 全球玩家帳號
- 今日活躍玩家（同一玩家／匿名裝置當日只計一次）
- 累積對局數
- 今日對局數
- 突破第 10 關玩家
- 突破第 20 關玩家
...
網頁開啟次數、今日網頁開啟次數及成功登入次數仍保留於後台統計，
但不再作為玩家前台的熱門程度指標。

### 預設挑戰者陪跑規則
- 預設挑戰者保有固定初始門檻。
- 分數只會緩慢向上，不會因低分新玩家加入而下降。
- 只有新玩家加入或真實玩家最高紀錄提升時，才可能前進一次。
- 單次最多增加目前分數的 3% 或 5,000 分，取較小值。
- 預設挑戰者不會改寫、借用或污染任何真實玩家資料。
- 真實玩家越多，前十名中的預設角色越少；10 位真實玩家時，預設角色退出前十名。

### 版本與授權維護備忘（開發文件）
以下內容屬於開發與發布前檢查，不在玩家前台呈現：
- 本作品不使用「Tetris」作為正式遊戲名稱。
- 公開發布前應持續檢查圖片、音效、字型、程式套件及第三方資源授權。
- 版權、隱私、資料保存及責任限制仍以遊戲內完整聲明與適用法律為準。


## V2.4.2 全球累積成果・九宮格統計

新增兩項由真實遊戲事件累積的公開數據：
- 全球累積消除行數：每次實際消除幾行，就累加幾行。
- 全球累積過關次數：每次成功完成一個關卡，就累加一次。

兩項數據包含正式玩家與訪客的有效遊戲事件，不會因 F5、重新登入或系統預設挑戰者而增加。
近期事件會先即時補入畫面，GitHub Actions 約每 5 分鐘將事件正式彙整至 `public_stats/summary`。
...
## V2.4.4 註冊缺漏引導・聲明視窗修正

- 建立帳號前檢查玩家帳號、挑戰密碼、再次輸入挑戰密碼、密碼提示與玩家命名規範確認。
- 缺漏時會在註冊視窗內顯示錯誤摘要，並自動捲動到第一個需要補填的位置。
- 修正免責聲明視窗版本文字仍停留舊版的問題。
- 強化「版權・免責聲明與玩家命名規範」連結文字。
...
- `index.html` 內 ES Module 的 JavaScript 語法。
- `scripts/update-public-stats.mjs` 語法。
- V2.4.5 版本與分享 Meta。
- 高等級續玩不再固定退回 Level 21。
- 分數、關卡與消行使用同一筆完整快照。
- Game Over 先顯示罩幕並恢復 `visibility`。
- 返回首頁、重開與本關重挑的二次確認入口。

## 歷史異常資料注意事項

V2.4.5 會阻止後續再把不同紀錄的分數、關卡與消行拼接，但不會自行猜測或改寫已存在的歷史資料。若某台裝置目前已保存 `2,306,634 分／Level 32／14 行`，而舊截圖顯示 `2,175,042 分／Level 32／192 行`，仍須先從排行榜備份、Retry History、舊存檔或雲端紀錄確認正確值，再另案進行資料修復。
